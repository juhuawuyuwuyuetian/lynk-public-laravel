...
class MakeAuthCommand extends Command
{
    ...
    {
        if (! is_dir(web_path('resources/views/layouts'))) {
            mkdir(web_path('resources/views/layouts'), 0755, true);
        }

        if (! is_dir(web_path('resources/views/auth/passwords'))) {
            mkdir(web_path('resources/views/auth/passwords'), 0755, true);
        }

        if (! is_dir(web_path('resources/views/auth/emails'))) {
            mkdir(web_path('resources/views/auth/emails'), 0755, true);
        }
    }
    ...
    protected function exportViews()
    {
        foreach ($this->views as $key => $value) {
            $path = web_path('resources/views/'.$value);
